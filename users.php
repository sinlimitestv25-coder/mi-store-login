...
$db = getDB();
$msg = ''; $tipo = '';
$rol_actual = $user['rol'] ?? 'final';
$roles_permitidos = [];
if ($rol_actual === 'admin') { $roles_permitidos = ['final','reseller','super_reseller']; }
elseif ($rol_actual === 'super_reseller') { $roles_permitidos = ['final','reseller']; }
elseif ($rol_actual === 'reseller') { $roles_permitidos = ['final']; }
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['accion'] ?? '') === 'crear') {
    ...
    $creditos_nuevos = (int)($_POST['creditos'] ?? 0);
    $vence = date('Y-m-d', strtotime('+' . $dias . ' days'));
    if (!in_array($rol, $roles_permitidos)) {
        $msg = 'No tienes permiso para crear ese tipo de usuario.'; $tipo = 'error';
    } elseif (!$nombre || !$email || !$pass) {
        $msg = 'Nombre, email y contrasena son obligatorios.'; $tipo = 'error';
    } else {
        $costo = ($rol === 'reseller') ? 1 : 0;
        $creditos_actuales = (float)($user['creditos'] ?? 0);
        if ($costo > 0 && $creditos_actuales < $costo) {
            $msg = 'No tienes suficientes creditos. Necesitas ' . $costo . ' credito(s).'; $tipo = 'error';
        } else {
            $hash = password_hash($pass, PASSWORD_DEFAULT);
            try {
                $db->beginTransaction();
                $db->prepare("INSERT INTO usuarios (nombre,email,password,rol,creditos,estado,creado_por,fecha_vence) VALUES (?,?,?,?,?,'activo',?,?)")
                   ->execute([$nombre, $email, $hash, $rol, $creditos_nuevos, $user['id'], $vence]);
                if ($costo > 0) {
                    $db->prepare("UPDATE usuarios SET creditos = creditos - ? WHERE id = ?")
                       ->execute([$costo, $user['id']]);
                    $_SESSION['usuario']['creditos'] = $creditos_actuales - $costo;
                }
                $db->commit();
                $user = currentUser();
                ...
                $db->rollBack();
                $msg = 'El email ya existe o ocurrio un error.'; $tipo = 'error';
            }
        }
    }
}
...
.bg{position:fixed;inset:0;background:radial-gradient(circle at 20% 20%,rgba(124,58,237,.18),transparent 28%),radial-gradient(circle at 80% 30%,rgba(16,185,129,.12),transparent 35%);z-index:0;}
.gl{position:fixed;inset:-10%;background-image:linear-gradient(rgba(178,102,255,.10) 1px,transparent 1px),linear-gradient(90deg,rgba(178,102,255,.10) 1px,transparent 1px);background-size:40px 40px;opacity:.28;z-index:1;}
...
.mini-pill{padding:8px 14px;border-radius:10px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#d8d8d8;font-size:13px;font-weight:bold;}
...
.stat-mini{background:rgba(14,14,18,.82);border:1px solid rgba(255,255,255,.08);border-radius:18px;padding:20px;text-align:center;}
...
.stat-mini .lbl{font-size:12px;color:#a0a0b0;text-transform:uppercase;letter-spacing:.05em;margin-top:4px;}
...
.sidebar h2,.main-panel h2{margin:0 0 6px;font-size:20px;font-weight:900;color:#b266ff;text-transform:uppercase;}
.sidebar p{margin:0 0 16px;color:#a9a9a9;font-size:14px;}
.input,.select-input{width:100%;padding:13px;border-radius:14px;border:1px solid rgba(255,255,255,.08);background:#151518;color:#fff;margin-bottom:12px;outline:none;font-size:14px;}
...
.users-table th{text-align:left;padding:12px 16px;font-size:12px;color:#a0a0b0;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid rgba(255,255,255,.06);}
.users-table td{padding:14px 16px;border-bottom:1px solid rgba(255,255,255,.04);font-size:14px;vertical-align:middle;}
...
.creditos-field{display:none;}
.costo-info{font-size:13px;margin-bottom:12px;padding:8px 12px;border-radius:10px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);}
.costo-gratis{color:#8df0bf;} .costo-pago{color:#fcd34d;} .costo-sr{color:#8dc8f0;}
...
<div class="container">
  ...
  <div class="stats-row">
    <div class="stat-mini"><div class="val"><?= $resellers ?></div><div class="lbl">RESELLERS</div></div>
    <div class="stat-mini"><div class="val"><?= number_format((float)($user['creditos'] ?? 0)) ?></div><div class="lbl">TUS CREDITOS</div></div>
    <div class="stat-mini"><div class="val"><?= $finales ?></div><div class="lbl">USUARIOS FINALES</div></div>
  </div>
  <div class="layout">
    <div class="sidebar">
      <h2>CREAR NUEVA CUENTA</h2>
      <p>Reseller cuesta 1 credito. Usuario final es gratis.</p>
      <form method="POST">
        ...
        <select class="select-input" name="rol" id="rol-select" onchange="actualizarForm(this.value)">
          <?php foreach($roles_permitidos as $r): ?>
          <option value="<?= $r ?>"><?= ucfirst(str_replace('_', ' ', $r)) ?></option>
          <?php endforeach; ?>
        </select>
        <div class="costo-info" id="costo-info"><span class="costo-gratis">&#10003; Usuario Final: GRATIS</span></div>
        <div class="creditos-field" id="creditos-field">
          <input class="input" type="number" name="creditos" id="creditos-input" placeholder="Creditos iniciales" min="0" value="0"/>
        </div>
    ...
    <div class="main-panel">
      ...
      <div class="empty-box">No has creado ningun usuario todavia.</div>
      <?php else: ?>
      <table class="users-table">
        <thead><tr><th>Nombre</th><th>Email</th><th>Rol</th><th>Creditos</th><th>Estado</th><th>Vence</th><th>Acciones</th></tr></thead>
        <tbody>
          ...
          <tr>
            ...
            <td><?= number_format((float)($u['creditos'] ?? 0)) ?></td>
...
function actualizarForm(rol){
  var info=document.getElementById('costo-info');
  var cf=document.getElementById('creditos-field');
  if(rol==='final'){info.innerHTML='<span class="costo-gratis">&#10003; Usuario Final: GRATIS, no consume creditos</span>';cf.style.display='none';}
  else if(rol==='reseller'){info.innerHTML='<span class="costo-pago">&#9888; Reseller: cuesta 1 credito de tu saldo</span>';cf.style.display='block';}
  else if(rol==='super_reseller'){info.innerHTML='<span class="costo-sr">&#9733; Super Reseller: asigna creditos iniciales</span>';cf.style.display='block';}
}
var rs=document.getElementById('rol-select');if(rs)actualizarForm(rs.value);
</script>
</body>
</html>
