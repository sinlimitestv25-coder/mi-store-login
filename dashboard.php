...
.session-bar{background:rgba(10,46,28,.60);border:1px solid rgba(16,185,129,.20);border-radius:14px;padding:12px 20px;margin-bottom:24px;font-size:13px;color:#8df0bf;}
...
.quick-card .label{font-size:13px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;}
.quick-card.g1{background:linear-gradient(135deg,rgba(124,58,237,.22),rgba(16,185,129,.12));}
.quick-card.g2{background:linear-gradient(135deg,rgba(16,185,129,.22),rgba(124,58,237,.12));}
.quick-card.g3{background:linear-gradient(135deg,rgba(178,102,255,.22),rgba(16,185,129,.12));}
.quick-card.g4{background:linear-gradient(135deg,rgba(245,158,11,.22),rgba(16,185,129,.12));}
.quick-card.g5{background:linear-gradient(135deg,rgba(178,102,255,.28),rgba(59,130,246,.15));}
.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:18px;margin-bottom:28px;}
.stat-card{background:rgba(14,14,18,.82);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:22px;transition:transform .18s;}
.stat-card:hover{transform:translateY(-3px);}
.stat-label{font-size:12px;color:#a0a0b0;text-transform:uppercase;letter-spacing:.07em;margin-bottom:8px;}
...
<div class="wrap">
  <div class="topbar">
    <div class="brand-area">
      <?php if(!empty($_SESSION['logo_url'])): ?><img src="<?php echo htmlspecialchars($_SESSION['logo_url']); ?>" alt="Logo" class="brand-logo"/><?php endif; ?>
  ...
  <div class="stats-grid">
    <div class="stat-card"><div class="stat-label">CLIENTES</div><div class="stat-value">0</div><div class="stat-desc">Control total de clientes registrados</div></div>
    <div class="stat-card"><div class="stat-label">SERVICIOS</div><div class="stat-value">0</div><div class="stat-desc">Catalogo global y servicios propios</div></div>
    <div class="stat-card"><div class="stat-label">USUARIOS</div><div class="stat-value">0</div><div class="stat-desc">Gestion de usuarios del sistema</div></div>
